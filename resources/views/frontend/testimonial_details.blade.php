...
@section('content')
<!-- breadcrumb-area -->
<title>{!!$system_settings->name!!} - {!!$testimonial->title!!}</title>
                ...
                <div class="breadcrumb__wrap__content">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{route('testimonial.details',$testimonial->slug)}}">{{$testimonial->title}}</a></li>
            ...
            <div class="col-lg-8">
                <div class="standard__blog__post">
                    <div class="standard__blog__thumb">
                        <img src="{{asset('backend/testimonial/')}}/{{!empty(($testimonial->testimonial_img))? $testimonial->testimonial_img : 'banner_img.png'}}" alt="{{$testimonial->title}}">
                    </div>
                    <div class="blog__details__content services__details__content">
                        <ul class="blog__post__meta">
                            <li><i class="fal fa-calendar-alt"></i>{{date("d-F-Y",strtotime($testimonial->created_at))}}</li>
                            <li><a href="http://{!!$testimonial->buyer_website!!}">Buyer Website: {!!$testimonial->buyer_website!!}</a></li>
                        </ul>
                        <h2 class="title">{!!$testimonial->title!!}</h2>
                        <h4 class="title">{!!$testimonial->short_title!!}</h4>
                        {!!$testimonial->short_des!!}

                        @if($testimonial->testimonial_multi_img!='')
                            ...
                            <div class="row">
                                @foreach($image_arr as $img)
                                    ...
                                    <div class="col-sm-4" @if($i>3 && $i%3==1) style="margin-top: 10px;" @endif>
                                        <img src="{{asset('backend/testimonial/')}}/{{!empty(($img))? $img : 'banner_img.png'}}" alt="{!!$testimonial->short_title!!}">
                                    </div>
                                     @endif
